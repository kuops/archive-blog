...
![](/img/gcr.io-image-sync/gcr-image-sync.png)
...
原理就是利用 gcloud 去获取 gcr.io 的镜像，然后通过 docker pull 和 docker tag 进行镜像生成，最后使用 docker push 上传至 dockerhub,脚本限制每次并发50个镜像。
```
#!/bin/bash
